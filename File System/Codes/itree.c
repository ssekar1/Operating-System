...
static int block_to_path(struct inode * inode, long block, int *offset)
{
	...
	// *offset = offset[level];  /* Return by reference */
   		
		do
			{
        
        if (Value_Num_Block > block)
        {
            level = Value_var - 1;
        }
        //return level;
		       ...
		       struct buffer_head *bh, int create)
{
	...
	if (level < 0)
		...
		*/
		 ...
		 * and possibly:
		 *   fs421_free_block(inode, Num_block, size)
		 *
...
/*
        ...
        /*while loop terminates when idata gets to a level that
         has no allocation (i.e. an allocation of 0).*/
        while (idata[alloc_level])
        {
        ...
        for (; alloc_level <= level; alloc_level++)
        {
            
            idata[alloc_level] = fs421_new_block(inode, 1 << alloc_level);
            
            added_level++;
            
            if (idata[alloc_level] == 0)
            {
        ...
        if (canAllocate == 0)
        {
	      ...
	      idata[alloc_level] = fs421_new_block(inode, (1 << l) - 1);
        ...
        // set to a negative errno (-ENOSPC makes most sense)
    }
